...
import { formatReviewMarkdown, formatReviewJson, orderComments } from '../../src/shared/output-format.js';
...
describe('formatReviewMarkdown', () => {
  ...
  it('groups comments by summary and file in diff order with excerpts', () => {
    const comments = [
      comment({ kind: 'lines', path: 'src/utils/clock.ts', side: 'new', start: 6, end: 6 }, 'Prefer 3600.'),
      comment({ kind: 'lines', path: 'src/services/user.ts', side: 'new', start: 18, end: 19 }, 'Why is the timeout hardcoded?'),
      comment({ kind: 'file', path: 'src/services/user.ts' }, 'Please add a unit test for the retry path.'),
      comment({ kind: 'summary' }, 'Split this into two PRs;\nthe migration should ship first.'),
      comment({ kind: 'lines', path: 'src/services/user.ts', side: 'old', start: 10, end: 10 }, 'Was this used elsewhere?'),
      comment({ kind: 'lines', path: 'src/services/user.ts', side: 'new', start: 1, end: 1 }, 'Import path changed.'),
    ];
  ...
  it('orders line comments with the same start line new before old', () => {
    const comments = [
      comment({ kind: 'lines', path: 'src/services/user.ts', side: 'old', start: 4, end: 4 }, 'old four'),
      comment({ kind: 'lines', path: 'src/services/user.ts', side: 'new', start: 4, end: 7 }, 'new four'),
    ];
  ...
  it('skips excerpt lines that are not part of the diff and keeps the marker', () => {
    const comments = [comment({ kind: 'lines', path: 'src/services/user.ts', side: 'new', start: 22, end: 25 }, 'gap')];
...
describe('formatReviewJson', () => {
  it('emits title, ordered comments and excerpts for line comments only', () => {
    const comments = [
      comment({ kind: 'lines', path: 'src/services/user.ts', side: 'new', start: 18, end: 19 }, 'timeout'),
    ...
    expect(parsed.comments[1].excerpt).toEqual(['+    const timeout = 3000;', '+    await refresh(token, timeout);']);
    expect(parsed.comments[1]).toMatchObject({ id: expect.any(String), target: { kind: 'lines', side: 'new' } });
  });
